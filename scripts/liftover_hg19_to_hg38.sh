...
path_to_downloaded_gwas=$4 # output directory specified in config
hg19ToHg38chain=$5
liftOver=$6
...
if [ -f ${path_to_downloaded_gwas}hg38/$trait/$trait.txt.gz ]; then
    rm ${path_to_downloaded_gwas}hg38/$trait/$trait.txt.gz
fi
if [ -f ${path_to_downloaded_gwas}hg38/$trait/$trait.txt.gz.tbi ]; then
